...
public class CsvSchemaParser : ISchemaParser
{
    ...
    public ClassSchema Parse(string fileContent, string fileName, string folderName)
    {
        ...
        var headers = lines[0].Split(Separator.CommaSeparator);
        var firstDataRow = lines.Length > 1 ? lines[1].Split(Separator.CommaSeparator) : null;
        ...
        {
            ClassName = $"{folderName}_{fileName}",
